...
sub dotProducts
{
  ...
  say OUT <<EOC;
static inline double dot_vec$n(const double* restrict a, const double* restrict b)
{
  double dot = 0.0;
  for(int i=0; i<$n; i++)
    dot += a[i]*b[i];
...
sub norms
{
  ...
  say OUT <<EOC;
static inline double norm2_vec$n(const double* restrict a)
{
  double dot = 0.0;
  for(int i=0; i<$n; i++)
    dot += a[i]*a[i];
...
sub vectorArithmetic
{
  ...
  my $vout = <<EOC;
// a + b
static inline void add_vec${n}_vout(const double* restrict a, const double* restrict b, double* restrict vout)
{
  for(int i=0; i<$n; i++)
    vout[i] = a[i] + b[i];
}
// a - b
static inline void sub_vec${n}_vout(const double* restrict a, const double* restrict b, double* restrict vout)
{
  for(int i=0; i<$n; i++)
    vout[i] = a[i] - b[i];
}
...
sub _getArg0
{
  ...
  my ($arg0) = $v =~ m/^static inline.*\(const double\* restrict ([a-z0-9_]+),/m or die "Couldn't recognize function in '$v'";
  return $arg0;
}
